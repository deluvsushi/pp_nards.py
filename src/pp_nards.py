...
class PPNards:
	...
	def update_profile(
			...
			language: str = None,
			avatar: str = None,
			global_tournaments_available: bool = True):
		data = {}
		if first_name:
	...
	def get_bonus(self, bonus_type: str = "first_enter_bonus"):
		return requests.get(
			f"{self.api}/api/account/bonus/{bonus_type}",
			headers=self.headers).json()

	def get_account_clubs(self):
		return requests.get(
			f"{self.api}/api/account/clubs",
			headers=self.headers).json()

	def get_sent_club_membership_requests(self):
		return requests.get(
			f"{self.api}/api/account/club-membership-requests",
			headers=self.headers).json()

	def join_to_club(self, short_id: int):
		return requests.get(
	...
	def get_tournament_status(self, tournament_id: str):
		return requests.get(
	...
	def enroll_tournament(self, tournament_id: str):
		return requests.post(
			f"{self.api}/api/tournaments/{tournament_id}/enroll",
			headers=self.headers).json()

	def get_tournament_prize_table(self, tournament_id: str):
		return requests.get(
	...
	def get_tournament_layers(self, tournament_id: str):
		return requests.get(
